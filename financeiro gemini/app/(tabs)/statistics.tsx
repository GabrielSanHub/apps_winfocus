...
export default function Statistics() {
  ...
  const loadStats = useCallback(() => {
    ...
    if (viewMode === 'monthly') {
        ...
        const data = getDashboardData(currentProfile.id, monthStr); // Removi 'monthly' se a função não aceitar, ou mantenha se aceitar
        
        // Ajuste conforme o retorno real da sua função getDashboardData
        setStats({
            income: data.income.total,
            expense: data.expense.total,
            balance: data.balance // ou data.income.total - data.expense.total
        });
    } else {
        // Pega dados gerais
  ...
  // Dados finais para o gráfico (evita erro de array vazio no componente)
